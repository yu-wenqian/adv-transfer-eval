...
class DropViT(nn.Module):
    def __init__(self,
                 img_size=224, patch_size=16, in_chans=3,  n_classes=1000, embed_dim=768, depth=12, n_heads=12,
                 mlp_ratio=4., qkv_bias=True,  p=0., drop_pos= 2, drop_prob= 0.15, drop_dim= 1
        ):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block1(
                dim=embed_dim, n_heads=n_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, p=p,
    ...
    def forward(self, x):
        ...
        #without dropping the first layer!
        if 3 in self.drop_dim:
            #drop half block
            prob = self.drop_prob / len(self.drop_dim)
            rand = prob + np.random.rand(23)
            rand = np.concatenate(([0], np.floor(rand)))
            drop_index = np.argwhere(rand > 0).flatten()

        if 4 in self.drop_dim:
            #drop block
            prob = self.drop_prob / len(self.drop_dim)
            rand = prob + np.random.rand(11)
            rand = np.concatenate(([0], np.floor(rand)))
...
def load_DropViT(args):
    custom_config = load_config(args.src_model)
    model = DropViT(**custom_config, drop_pos= args.drop_pos, drop_prob= args.drop_prob,
