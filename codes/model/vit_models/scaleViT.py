...
class ScaleViT(nn.Module):
    ...
    def rescale_x(self, x, stride = 2, kernel_size = 2, mode = 'avg'):
        batch_size, num_patches, hid_dim = x.shape()
        class_token = x[:, 0, :]
        num_patch_per_side = int(math.sqrt(num_patches-1))
        resized_x_tmp = torch.reshape(x[:, 1:, :], (batch_size, num_patch_per_side, num_patch_per_side,hid_dim))
        resized_x = torch.view(resized_x_tmp, (batch_size, hid_dim, num_patch_per_side, num_patch_per_side))
        resized_x_exp = torch.reshape(x[:, 1:, :], (batch_size, hid_dim, num_patch_per_side, num_patch_per_side))
        assert resized_x_exp.data == resized_x_exp.data
        if mode == 'avg':
            pooling = torch.nn.AvgPool2d(kernel_size, stride)
        elif mode == 'max':
            pooling = torch.nn.MaxPool2d(kernel_size, stride)
        output = pooling(resized_x_exp)
        x = torch.reshape(output, (batch_size, None, hid_dim))
...
def load_ScaleViT(args):
    custom_config = load_config(args.src_model)
    model = ScaleViT(**custom_config, drop_prob = args.drop_prob, drop_dim=args.drop_dim, drop_pos = args.drop_pos,num_perm = args.num_perm,
