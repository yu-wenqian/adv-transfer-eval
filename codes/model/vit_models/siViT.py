...
class SIViT(nn.Module):
    def __init__(
        ...
        self.kernel_min = args.range[0]
        self.kernel_max = args.range[1]
        widths = list(range(min, max+1))
        self.transforms = [transforms.Resize((x * 16, x * 16)) for x in widths]

        print(self.mode, self.layers, self.kernel_min, self.kernel_max, )
        ...
        for i, block in enumerate(self.blocks):
            if i in self.layers:
                if self.random:
                    ...
                    _x = self.rescale_1d(_x, kernel_size= kernel_size, mode = self.mode)
                # else:
                #     _x = self.rescale_1d(_x, kernel_size= self.kernel_size)
...
def load_siViT(args):
    ...
    model = SIViT(**custom_config, args = args)
    model.eval()
