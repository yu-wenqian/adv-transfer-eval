...
class GhostViT(nn.Module):
    def __init__(
        ...
        self.depth = depth
        print(self.prob_skip_eros, self.prob_drop_layer)
...
def load_ghostViT(model_name, args):
    custom_config = load_config(model_name)
    model = GhostViT(**custom_config,  prob_skip_eros = args.skip_eros, prob_drop_layer = args.drop_layer)
    model.eval()

    model_official = load_official_model(model_name[5:])
