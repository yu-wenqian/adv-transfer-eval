...
class ActorCritic(nn.Module):
    def __init__(self, feature_dim, state_dim, hidden_state_dim=1024, policy_conv=True, action_std=0.1):
        ...
        self.action_var = torch.full((2,), action_std).to(device)
    ...
    def act(self, state_ini, memory, restart_batch=False, training=False):
        if restart_batch:
            del memory.hidden[:]
            memory.hidden.append(torch.zeros(1, state_ini.size(0), self.hidden_state_dim).to(device))
        ...
        if training:
            ...
            action_logprob = dist.log_prob(action).to(device)
    ...
    def evaluate(self, state, action):
        ...
        state, hidden = self.gru(state, torch.zeros(1, batch_size, state.size(2)).to(device))
        state = state.view(seq_l * batch_size, -1)
        ...
        action_logprobs = dist.log_prob(torch.squeeze(action.view(seq_l * batch_size, -1))).to(device)
...
class Full_layer(torch.nn.Module):
    ...
    def forward(self, x, restart=False):
        ...
        if self.fc_rnn:
            if restart:
                output, h_n = self.rnn(x.view(1, x.size(0), x.size(1)), torch.zeros(1, x.size(0), self.hidden_state_dim).to(device))
                self.hidden = h_n
            else:
